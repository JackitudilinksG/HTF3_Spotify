...
export const verifyAdminCode = async (adminCode: string) => {
    try {
        if (!process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID) {
            throw new Error('Missing database ID');
        }
        ...
        console.log('Verifying admin code:', adminCode);
        ...
        console.log('Appwrite admin response:', response);

        if (response.documents.length > 0) {
            console.log('Found admin:', response.documents[0]);
            return {
                success: true,
                admin: response.documents[0]
            };
        }

        console.log('No admin found with code:', adminCode);
        return {
            success: false,
            error: 'Invalid admin code'
        };
    } catch (error: any) {
        console.error('Detailed error verifying admin code:', error);
        return {
            success: false,
            error: error.message || 'Failed to verify admin code'
        };
    }
}; 